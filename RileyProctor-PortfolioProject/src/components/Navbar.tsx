...
const Navbar = () => {
    ...
    const navbarHeadings = (
        <ul className="sm:text-sm flex flex-col gap-8 text-gray-700 font-medium text-xl dark:text-gray-300 dark:shadow-white">
    ...
    return (
        <nav
            ...
            <div className="max-w-6xl mx-auto  py-4 flex justify-between items-center">
                <div className="sm:text-sm text-2xl font-bold">Riley Proctor │ Gameplay Programmer</div>
                <ThemeSwitchButton/>

                {/*If device viewport is larger than phone size, then display headings in navbar*/}
                {window.innerWidth < medBreakpoint ? (
                    navbarHeadings
                ) : (
                    <BurgerMenu list={navbarHeadings} />
                )}
